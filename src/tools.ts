...
export function createDispatchTasksTool(client: DeviceBridge) {
  return {
    ...
    description: [
      ...
      'Use this for ANY task that takes more than about a minute — browsing,',
      'recording, multi-post work — EVEN ON A SINGLE DEVICE, and whenever two or',
      'more phones are involved, or the task repeats an',
      ...
      'SIZE EACH TASK SMALL: a phone spends ~20s per step and has a 100-step',
      'budget per task; browsing one post and recording it costs 6-10 steps, so',
      'cap a browsing task at ~5 posts (~50 steps). A "browse 10 posts with full',
      'records" task burns the whole budget and comes back as stuck. Prefer more',
      'rounds of small tasks over one big task per device.',
    ].join(' '),
    parameters: {
      type: 'object',
      properties: {
...
export function createJobStatusTool(client: DeviceBridge) {
  return {
    ...
    description: [
      ...
      'DO NOT WAIT ON FAILED DEVICES: a device that reports NO_FIRST_STEP or any',
      'other failure is out of this round — move on with the results of the',
      'devices that finished and re-plan the missing share, instead of polling',
      'until every device ends. A wedged phone once held a whole round for 72',
      'minutes at zero steps; it now fails within ~3 minutes and that failure is',
      'final for the round.',
    ].join(' '),
    parameters: {
      type: 'object',
